...
rdata_ctx_t *rdata_ctx_init(rdata_io_t *io, const char *filename) {
    ...
    if (rdata_machine_is_little_endian()) {
...
rdata_error_t rdata_parse(rdata_parser_t *parser, const char *filename, void *user_ctx) {
    ...
    if (ctx->machine_needs_byteswap) {
        v2_header.format_version = rdata_byteswap4(v2_header.format_version);
        v2_header.writer_version = rdata_byteswap4(v2_header.writer_version);
        v2_header.reader_version = rdata_byteswap4(v2_header.reader_version);
    }
...
static rdata_error_t read_sexptype_header(rdata_sexptype_info_t *header_info, rdata_ctx_t *ctx) {
    ...
    if (ctx->machine_needs_byteswap)
        sexptype = rdata_byteswap4(sexptype);
    ...
    if (header.type == RDATA_SEXPTYPE_PAIRLIST) {
        if (header.attributes) {
            ...
            if (ctx->machine_needs_byteswap)
                header_info->attributes = rdata_byteswap4(header_info->attributes);
        }
        if (header.tag) {
        ...
        if (val_info.header.type == RDATA_SEXPTYPE_INTEGER_VECTOR) {
            ...
            if (length <= sizeof(ctx->dims)/sizeof(ctx->dims[0])) {
                ...
                if (ctx->machine_needs_byteswap) {
                    int i;
                    for (i=0; i<length; i++) {
                        ctx->dims[i] = rdata_byteswap4(ctx->dims[i]);
                    }
                }
                if (ctx->dim_handler) {
...
static rdata_error_t read_compact_intseq(const char *name, rdata_ctx_t *ctx) {
    ...
    if (ctx->machine_needs_byteswap) {
        vals[0] = rdata_byteswap_double(vals[0]);
        vals[1] = rdata_byteswap_double(vals[1]);
        vals[2] = rdata_byteswap_double(vals[2]);
    }
        ...
        if (ctx->machine_needs_byteswap) {
            if (input_elem_size == sizeof(double)) {
                ...
                for (i=0; i<buf_len/sizeof(double); i++) {
                    d_vals[i] = rdata_byteswap_double(d_vals[i]);
                }
            } else {
                uint32_t *i_vals = (uint32_t *)vals;
                for (i=0; i<buf_len/sizeof(uint32_t); i++) {
                    i_vals[i] = rdata_byteswap4(i_vals[i]);
                }
            }
        }
    }
